...
def get_matchup_from_api(team_a, team_b):
    try:
        ...
        st.warning(f"API unavailable, using local Streamlit logic instead. Error: {e}")
        return None
...
LOGO_DIR = Path(r"C:\Users\ozoma\PyCharmProjects\PythonProject1\assets\logos")
...
# RENDER HELPERS
# ============================================================
def team_logo_html(team_name, size=42):
    logo = get_logo_path(team_name)
    if not logo:
        return ""
    encoded = base64.b64encode(Path(logo).read_bytes()).decode()
    return f"<img src='data:image/png;base64,{encoded}' style='height:{size}px; width:{size}px; object-fit:contain;'>"
...
<div>{row1["date"]}</div>
<div class="{winner_class}">{winner}</div>
<div><span class="score-red">{pts1}</span><span class="score-sep">--</span><span class="score-blue">{pts2}</span></div>
...
def render_matchup_history(team_games_df, team_a, team_b, limit=5):
    matchup_games = team_games_df[
        (
            ((team_games_df["team"] == team_a) &
             (team_games_df["opponent"] == team_b))
            |
            ((team_games_df["team"] == team_b) &
             (team_games_df["opponent"] == team_a))
        )
    ].copy()
    ...
    matchup_games = matchup_games.sort_values("date")

    st.markdown("<div class='pro-card'>", unsafe_allow_html=True)
    st.markdown("<h2>Recent Matchup History</h2>", unsafe_allow_html=True)
    ...
    for _, row in matchup_games[::-1].iterrows():
        game_id = row["game_id"]
        ...
        margin = int(team1["pts"]) - int(team2["pts"])

        st.markdown(f"""
        <div class='history-row'>
            <div>{team1['date']}</div>
            <div>{team1['team']}</div>
            <div><span class='score-red'>{int(team1['pts'])}</span> - <span class='score-blue'>{int(team2['pts'])}</span></div>
            <div>{team2['team']}</div>
            <div class='margin-green'>{margin:+d}</div>
        </div>
        """, unsafe_allow_html=True)

        displayed_games += 1

        if displayed_games >= limit:
            break
...
<div class="subtitle">Team profiles, projected lineups, player roles, matchup factors, and scouting notes.</div>
...
with hero_cols[0]:
    ...
    st.markdown(f'<h2 style="color:{color_a}!important;">{team_a}</h2>', unsafe_allow_html=True)
with hero_cols[1]:
    st.markdown('<div class="app-kicker">HEAD-TO-HEAD REPORT</div>', unsafe_allow_html=True)
    st.markdown(f'<h2>{team_a} vs {team_b}</h2>', unsafe_allow_html=True)
    st.markdown(f'<h3 style="color:#D6A84F!important;">EDGE: {edge}</h3>', unsafe_allow_html=True)
    st.markdown(f'<div style="font-family:Oswald;font-size:24px;font-weight:800;">CONFIDENCE: {confidence}%</div>', unsafe_allow_html=True)
with hero_cols[2]:
    if logo_b_uri:
        st.image(logo_b_uri, width=110)
    else:
        st.markdown(initials_badge(team_b, color_b), unsafe_allow_html=True)
    st.markdown(f'<h2 style="color:{color_b}!important;">{team_b}</h2>', unsafe_allow_html=True)
